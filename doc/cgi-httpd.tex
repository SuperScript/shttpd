...
\title{shttpd}
\begin{document}
\section{The \cmd{cgi-httpd} program}
...
executing \cvar{prog}, \cmd{cgi-httpd} sets the execution path to \cmd{/bin}.
...
For a request specifying the hostname \cvar{H}, \cmd{cgi-httpd}
...
\begin{code}
  cgi-config shttp shttplog /usr/local/shttpd/service/cgi-example \var{port} cgi-example
\end{code}
...
For each \cvar{hostname} you wish accept in requests, create the
directory
\cmd{/usr/local/shttpd/service/cgi-example/root/host/\var{hostname}}.
