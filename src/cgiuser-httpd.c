...
#define FATAL "cgiuser-httpd: fatal: "

void usage(void)
{
  strerr_die1x(21,"cgiuser-httpd: usage: cgiuser-httpd child");
}
...
void get(char * const *cmd)
{
  ...
  if (!pathexec_env("SERVER_SOFTWARE","cgiuser-httpd")) _exit(21);  
  tai_now(&now);
  if (!httpdate(&nowstr,&now)) _exit(21);
